...
class TensorboardHookModified(spk.train.TensorboardHook):
    ...
    def on_batch_end(self, trainer, train_batch, result, loss):
        ...
        if self._counter % self.each_n_batch == 0:
            print(loss, file=open(os.path.join(self.log_path, "loss.log"), "a"))
...
def train(args, model, train_loader, val_loader, device):
    ...
    try:
        ...
        torch.save(model, "failed_model.pt")
        raise e
...
if __name__ == '__main__':
    ...
    if not args.datapath:
        args = spk.utils.read_from_json("/home/jovyan/khrabrov/SchNOrb/src/scripts/default_args.json")
